...
- Action: Enter end date into the field
...
def precheck(**kwargs) -> Tuple[bool, str]:
    """Verify search page is displayed before setting end date."""
...
def verifier(end_date: str = "", **kwargs) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
    ...
    field_config = get_element("search_page", "end_date_field")
    ...
    if extracted_date:
        ...
        if similarity >= 0.80:
            return True, f"End date verified ({similarity:.0%} match)", verification_data
        else:
            return False, f"End date mismatch (similarity: {similarity:.0%})", verification_data
    else:
        verification_data = {
            "expected_date": end_date,
            "extracted_text": extracted_text,
...
def error_handler(error_msg: str, attempt: int, max_attempts: int, **kwargs) -> Tuple[bool, str]:
    """Handle errors specific to setting end date."""
    ...
    return False, f"Failed to set end date after {max_attempts} attempts"
