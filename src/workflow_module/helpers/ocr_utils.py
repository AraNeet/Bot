...
def test_easyocr_structure(image: np.ndarray) -> None:
    """
    Test function to verify EasyOCR is working correctly.
    ...
    try:
        ...
        print(f"[OCR] EasyOCR test completed successfully")
        print(f"[OCR] Found {len(results) if results else 0} text elements")
        
    except Exception as e:
        print(f"[OCR ERROR] Test failed: {e}")


def debug_easyocr_output(image: np.ndarray) -> None:
    """
    Debug function to verify EasyOCR output.
    ...
    try:
        ...
        print(f"[OCR] Debug completed successfully")
        print(f"[OCR] Found {len(results) if results else 0} text elements")
        
    except Exception as e:
        print(f"[OCR ERROR] Debug failed: {e}")