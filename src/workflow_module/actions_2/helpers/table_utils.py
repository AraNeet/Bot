...
def search_current_view(target_texts: List[str], deal_number: str, crop_x: int, crop_y: int, 
    ...
    try:
        ...
        # Found the row - find the deal_number position specifically
        ...
        if not deal_number_position and positions:
            deal_number_position = positions[0]
            print(f"[SEARCH_VIEW] Using first matched position as deal_number: {deal_number_position}")
        
        if not deal_number_position:
            return False, "Could not determine deal_number position", None
        
        x, y, w, h = deal_number_position
        screen_x = x + crop_x
        screen_y = y + crop_y
        ...
        # Define search region - look for RowExpander in the same row as deal_number
        # Search horizontally across the table at the same Y level as the deal_number
        ...
        if found and expander_position:
            ...
            if abs(exp_y - screen_y) <= h * 1.5:  # Allow some tolerance
                ...
                print(f"[SEARCH_VIEW] Found RowExpander at ({click_x}, {click_y}) with confidence {confidence:.2f} (same row as deal_number)")
                ...
                print(f"[SEARCH_VIEW] RowExpander found but Y mismatch (expander: {exp_y}, deal_number: {screen_y}), using deal_number position")
        
        # Fallback to deal_number position - click slightly to the left of the deal number text
        click_x = screen_x - 50  # Click to the left of the deal number
        click_y = screen_y + h // 2  # Center vertically on the deal number row
        print(f"[SEARCH_VIEW] RowExpander not found or wrong row, using deal_number row position ({click_x}, {click_y})")
        match_info['click_x'] = click_x
        match_info['click_y'] = click_y
        match_info['button'] = 'right'
        return True, f"Row found at deal_number row position ({click_x}, {click_y})", match_info
